...
- Every `bundle_root_artifacts` declaration is required at final validation: `output_name` selects the delivered path, with the source basename as the fallback. The file must remain inside the bundle and be a regular file, not a symlink or hardlink. This checks delivery completeness, not runtime success or the existence of original build-source paths.
