...
- False-positive and non-security entries should state the boundary-oriented reason code when known: `expected_behavior`, `outside_security_boundary`, `requires_non_default_admin_trust`, `default_config_not_vulnerable`, `insufficient_attacker_condition`, or `insufficient_security_impact`.
...
- Include P5 gate outcomes when available: `audit-disposition.json` status, OMC runtime hygiene status, sandbox preflight status, Docker strict clean, finalization/assert status, and confirmed report quality gate status when confirmed bundles exist.
...
误报 / 非安全缺陷：
- <候选项 1>：<为什么不是漏洞 / 为什么只是非安全缺陷>。原因代码：<expected_behavior / outside_security_boundary / requires_non_default_admin_trust / default_config_not_vulnerable / insufficient_attacker_condition / insufficient_security_impact>。记录位置：<audit-workspace>/false-positives.md
...
- P5 门禁状态：audit-disposition=<ok/blocked/missing>；runtime hygiene=<clean/attention_needed/not_applicable>；sandbox preflight=<passed/rejected_unsafe_sandbox/not_applicable>；finalization/assert=<passed/blocked>。
...
False positives / non-security defects:
- <Candidate 1>: <why it is not a real vulnerability / why it is only a non-security defect>. Reason code: <expected_behavior / outside_security_boundary / requires_non_default_admin_trust / default_config_not_vulnerable / insufficient_attacker_condition / insufficient_security_impact>. Record: <audit-workspace>/false-positives.md
...
- P5 gate status: audit-disposition=<ok/blocked/missing>; runtime hygiene=<clean/attention_needed/not_applicable>; sandbox preflight=<passed/rejected_unsafe_sandbox/not_applicable>; finalization/assert=<passed/blocked>.
